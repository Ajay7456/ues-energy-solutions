...
export default function Home() {
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* Services Overview - HORIZONTAL LAYOUT */}
          ...
          <div className="space-y-6">
            {services.map((service) => (
              <Link 
                key={service.id}
                href={service.link}
                className="group bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 flex items-start h-auto"
              >
                {/* Image Container - Left side, reduced size for mobile */}
                <div className="flex-shrink-0 w-32 h-32 md:w-40 md:h-40 m-4 md:m-6 flex items-center justify-center">
                  <div className="w-full h-full bg-white rounded-lg shadow-sm border border-blue-100 overflow-hidden p-2 group-hover:scale-105 transition-transform duration-500">
                    <img
                      src={service.image}
                      alt={service.name}
                ...
                {/* Text Content - Right side, aligned with image */}
                <div className="flex-grow p-4 md:p-6 flex flex-col justify-center h-full">
                  <h3 className="text-lg md:text-xl font-bold text-gray-900 mb-2 md:mb-3 group-hover:text-blue-900 transition-colors">
                    {service.name}
                  </h3>
                  <p className="text-gray-600 mb-3 md:mb-4 text-sm md:text-base">
                    {service.desc}
                  </p>
                  <div className="flex items-center text-blue-900 font-medium text-sm md:text-base group-hover:text-blue-700 transition-colors mt-1 md:mt-2">
