...
export default function ContactFormModal({ isOpen, onClose }: ContactFormModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // In production, you would send this to an API route
    const formData = {
      to: 'ceo@uesenergysolutions.com',
      subject: 'New Quote Request from UES Website',
      name: form.name,
      email: form.email,
          ...
          {submitted ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <p className="text-xs text-gray-500 text-center mt-4">
                Your message will be sent to ceo@uesenergysolutions.com
              </p>
            </form>
          )}
        </div>
